...
class ReliabilityGateway:
    ...
    def __init__(
        ...
        self.cache = cache
        # In-memory fallback cache in case central SharedRedisCache goes down (Graceful Degradation)
        self._fallback_cache = ResponseCache(ttl_seconds=300, similarity_threshold=0.92)
    ...
    def complete(self, prompt: str) -> GatewayResponse:
        ...
        if self.cache is not None:
            try:
                ...
                # central cache down, gracefully fall back to local in-memory cache
                cached_text, score = self._fallback_cache.get(prompt)
            ...
            try:
                ...
                if self.cache is not None:
                    try:
                        ...
                        self._fallback_cache.set(prompt, response.text, {"provider": provider.name})
